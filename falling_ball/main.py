    ...
    while running:
        ...
        if mask is not None:
            ...
            for (x1, y1), (x2, y2) in new_lines:
                line = pymunk.Segment(space.static_body, (x1, y1), (x2, y2), 5)  # Increased line thickness
                line.elasticity = 0.8  # Increased elasticity
                line.friction = 1.5    # Increased friction
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                ball_body = pymunk.Body(10, pymunk.moment_for_circle(10, 0, 10))  # Increased mass
                ball_body.position = x, y
                ball_shape = pymunk.Circle(ball_body, 10)
                ball_shape.elasticity = 0.9  # Increased elasticity
                ball_shape.friction = 1.0    # Increased friction
                space.add(ball_body, ball_shape)
                balls.append((ball_body, ball_shape))

        # Set the black background for Pygame window
        screen.fill((0, 0, 0))

        # Draw contours on the original resized frame before rendering to Pygame
        for (x1, y1), (x2, y2) in new_lines:
...
video_url = "http://192.168.43.1:8080/video"  # Use your actual video URL here
resize_factor = 0.5
